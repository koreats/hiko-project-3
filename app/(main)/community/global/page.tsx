...
export default function GlobalCommunityPage() {
  ...
  const loadPosts = async (reset = false) => {
    ...
    try {
      const newPosts = await getCommunityPosts("글로벌")
      ...
      <div className="bg-white p-4 border-b sticky top-0 z-10 backdrop-blur-sm bg-white/95">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-text-primary">글로벌 커뮤니티</h1>
              <p className="text-xs text-text-secondary">전 세계 친구들과 소통해보세요</p>
            </div>
          </div>
      ...
      <div className="bg-white">
        {loading && posts.length === 0 ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <h3 className="text-lg font-medium text-text-primary mb-2">아직 등록된 글이 없어요</h3>
            <p className="text-text-secondary mb-6">첫 번째 글을 작성하고 전 세계 친구들과 인사를 나눠보세요!</p>
            <Link href="/community/new">
              <Button>
